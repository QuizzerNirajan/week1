...
template sudoku() {
    ...
    component pRange[9][9];
    component sRange[9][9];
    
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
  pRange[i][j] = RangeProof(32);
            sRange[i][j] = RangeProof(32);

            pRange[i][j].in <== puzzle[i][j];

            pRange[i][j].range[0] <== 0;
            pRange[i][j].range[1] <== 9;

            sRange[i][j].in <== solution[i][j];

            sRange[i][j].range[0] <== 0;
            sRange[i][j].range[1] <== 9;

            assert(pRange[i][j].out == 1);
            assert(sRange[i][j].out == 1);
